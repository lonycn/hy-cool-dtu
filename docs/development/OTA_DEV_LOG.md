...
│   ├── build.sh                  # 构建脚本
│   ├── flash.sh                  # 烧录脚本
│   ├── detect_device.sh          # 设备检测
│   ├── test_chip_connection.sh   # 连接测试
│   └── connect_nano100sd3bn.sh   # 专用连接脚本
├── debug/                        # 调试工具
└── test/                         # 测试工具
...
2. **备份现有项目**
   ...
   ```bash
   cp -r "Nano/modbus_ver3.4.2(3283)/App" "App_backup"
   ```
...
# 查看现有项目文件
find 'Nano/modbus_ver3.4.2(3283)/App' -name "*.c" | head -5
```
