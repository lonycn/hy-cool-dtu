...
// 全局变量定义
unsigned char WatchDogTestFlag = 0; /*!< 看门狗测试标志 */
unsigned char ResetFlag = 0;        /*!< 复位标志 */
unsigned char Interval = 60;        /*!< 时间间隔 */
...
void Main_loop_Initial() {
    ...
    // 🔧 OTA模块初始化
    ota_config_t ota_config = {.server_url = "https://ota.coolstorage.com",
...
void SelectMotorTemp() {
    ...
    if (SelType != 0) {
        setTemp =
            ParaList[cSelectMenu * 2 + (SelType - 1) * 4 + 1] + ParaList[cSelectMenu * 2 + (SelType - 1) * 4] * 256;
        setError =
            ParaList[cSelectMenu * 2 + (SelType - 1) * 4 + 3] + ParaList[cSelectMenu * 2 + (SelType - 1) * 4 + 2] * 256;
...
void Main_loop_while() {
    ...
    if (++ota_check_counter >= 5) {  // 5*2秒 = 10秒
        ota_check_counter = 0;
