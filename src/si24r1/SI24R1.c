...
//unsigned char  TX_ADDRESS[5] = { 0xf0,0x5f, 0x4a, 0x55, 0x00 };
//unsigned char  TX_ADDRESS[5] = { 0xf0,0x00, 0x55, 0x4a, 0x5f };
unsigned char  TX_ADDRESS[5] = { 0xf0,0x00, 0x55, 0x4a, 0x5f };
#define freq  9
...
{
	CE=0;
	SI24R1_Write_Reg(WRITE_REG + SETUP_AW, TX_ADR_WIDTH-2);
	...
	SI24R1_Write_Reg(WRITE_REG + RF_CH, freq);                 						// 选择射频通道0x40
	SI24R1_Write_Reg(WRITE_REG + RX_PW_P0, TX_PLOAD_WIDTH+2);  						// 接收通道0选择和发送通道相同有效数据宽度
	SI24R1_Write_Reg(WRITE_REG + RF_SETUP, 0x27);//250k            						// 数据传输率2Mbps，发射功率7dBm
	SI24R1_Write_Reg(WRITE_REG + CONFIG, 0x07);//0f-07              						// CRC使能，16位CRC校验，上电，接收模式
...
unsigned char SI24R1_RxPacket(unsigned char *rxbuf)
{
	...
	{
		SI24R1_Read_Buf(RD_RX_PLOAD,rxbuf,32);//TX_PLOAD_WIDTH);     //读取数据
...
{        
 unsigned char i;
		if(!SI24R1_RxPacket(buf1))
		{     	
		 #ifdef   __Nano100
		   SendDataToBus1_uart0(buf1,TX_PLOAD_WIDTH);
		 #endif
...
uint64_t id= 21110601001;
#define cDoorMagicType  3
#define cWireless       1
...
void set_tx_packet(TXPACKET_FORMAT* packet,int16_t temperature,int16_t humidity,int8_t bat_level,uint8_t rec_on,uint16_t cnt)
{
	...
	if(id>21110601040)
		  id=21110601001;
#endif
	packet->CONTENT = cWireless;   //
	packet->version = cDoorMagicType;
	
	packet->tag_id =(ParaList[cid*2]*256+ParaList[cid*2+1])*10000;
	packet->tag_id +=ParaList[cid*2+2]*256+ParaList[cid*2+3];
	packet->tag_id *=1000;
	packet->tag_id +=ParaList[cid*2+4]*256+ParaList[cid*2+5];
...
void SetDoorMagicPacakage(uint8_t status)
 {uint8_t *ptr,i,j;
   if(status==2)  //open
      set_tx_packet(&tx_packet,1,0,7,1,10000);//设置数据包
   else if(status==1)
    set_tx_packet(&tx_packet,1,1,7,1,10000);//设置数据包
 else 
    set_tx_packet(&tx_packet,0,0,7,1,10000);//设置数据包   
