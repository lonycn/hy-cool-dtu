...
void DealWithSingleRegWrite_06(unsigned char *SendTempBuffer) {
    ...
    */
    ...
    if (i < 2010) {
        ...
        if ((i == cTempOffset) || (i == cTempOffset + 1)) {
            ...
            if ((SendTempBuffer[0] >= AddrStart) && (SendTempBuffer[0] < AddrStart + AddrLen) &&
                ...
                ParaList[(i - cTempOffset) * 2 + (SendTempBuffer[0] - AddrStart) * 4 + cLoraNodeOffset * 2] =
                    SendTempBuffer[4];
                ParaList[(i - cTempOffset) * 2 + (SendTempBuffer[0] - AddrStart) * 4 + cLoraNodeOffset * 2 + 1] =
                    SendTempBuffer[5];
                Program();
            }
#else
        ...
        //-------------------lora  ��д
        else if ((i == cLoraReadConfigPara) &&
                 ((SendTempBuffer[5] == 0x01) || (SendTempBuffer[5] == 0x02)))  // ��ȡlora����
        {
            ParaList[i * 2] = SendTempBuffer[4];
            ParaList[i * 2 + 1] = SendTempBuffer[5];
        ...
        else if ((i == cLoraReadConfigPara) && (SendTempBuffer[5] == 0x00))  // �˳���ȡ
        {
            ...
            ParaList[i * 2 + 1] = SendTempBuffer[5];
            SwtichToWorkmode();
        } else if ((i == cLoraWriteConfigPara) &&
                   ((SendTempBuffer[5] == 0x01) || (SendTempBuffer[5] == 0x02)))  // ����lora����
        {
            ParaList[i * 2] = SendTempBuffer[4];
            ParaList[i * 2 + 1] = SendTempBuffer[5];
    ...
    /*���õ���ʪ��10��ƫ��У׼ֵ,�趨�ĵ�ַ��ʼֵ,��ַ����(���16���ڵ�),lora����
       3000���ϵ�ַ�Ĵ���*/
    // 3000---3019
    else if (i >= cTempOff) {
        if (ParaList[23] == 78) {
    ...
    if (flag == 0) {
        ...
        for (i = 0; i < cPageRecordNum / 2; i++) {
            ...
            sprintf(TemBufferSend, "%d-%d-%d  %d:%d:%d  %d-%d-%d-%d  Temp:%d  Humi:%d \r\n\0\0\0", TemBuffer[m],
                    TemBuffer[m + 1], TemBuffer[m + 2], TemBuffer[m + 3], TemBuffer[m + 4], TemBuffer[m + 5],
        ...
        if ((SendTempBuffer[0] >= AddrStart) && (SendTempBuffer[0] < AddrStart + AddrLen) &&
            ...
            for (k = i; k < i + SendTempBuffer1[2] / 2; k++) {
                SendTempBuffer1[j++] = ParaList[(k - cTempOffset) * 2 + (SendTempBuffer[0] - AddrStart) * 4 +
                                                cLoraNodeOffset * 2];  // ��ʪ��λ��+�豸��Ӧλ��+ƫ��baseλ��
                SendTempBuffer1[j++] =
